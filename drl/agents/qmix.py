...
class QMIX(Agent):
    def __init__(self, embryo):
        super(QMIX, self).__init__(embryo)
    ...
    def is_learn(self):
        return self.embryo.is_ready and self.embryo.memory.push_status
    
    def predict(self, agents_obs, agents_action):
        pred_Q = self.embryo.execute(np.array(agents_obs)).cpu().detach().squeeze()
        agents_action = torch.LongTensor(np.array(agents_action))
        pred_Q[agents_action==0] = -1e10
    ...
    def policy(self, agents_obs, agents_action, explore_step=None):
        ...
        if self.embryo.warmup_full_random and not self.is_learn() and explore_step is None:
            ...
            return self.embryo.epsilon_scheduler.explore(explore_step, random_action_func, function(self.predict, agents_obs, agents_action))
